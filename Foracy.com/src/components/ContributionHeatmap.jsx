...
export default function ContributionHeatmap({ 
  ...
  const currentYear = selectedYear !== null ? selectedYear : internalSelectedYear;
  ...
  const getYearData = (year) => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const count = typeof dateData === 'object' ? (dateData?.total || 0) : (dateData || 0);
        let level = 0;
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="flex items-center gap-3">
        <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">年份：</span>
            ...
            <div className="flex items-start gap-2">
              <div className="flex flex-col gap-[3px] mt-1 text-xs text-gray-500 dark:text-gray-400 select-none w-8">
                {dayLabels.map((day, idx) => (
                  <div key={`d-${idx}`} className="h-3 flex items-center justify-end pr-1">
                    {idx % 2 === 0 ? day : ""}
                  </div>
                ))}
              </div>

              <div className="inline-flex gap-[3px] select-none" aria-label="Contribution heatmap">
                {columns.map((col, i) => (
